// 记录(Record)/提交(Submission)模型 ，用于管理在线评测系统中的用户提交记录
...
export default class RecordModel { // 核心类
    ...
    static PROJECTION_LIST: (keyof RecordDoc)[] = [ // 查询时返回的字段列表
        '_id', 'score', 'time', 'memory', 'lang',
        'uid', 'pid', 'rejudged', 'progress', 'domainId',
        'contest', 'judger', 'judgeAt', 'status', 'source',
        'files', 'hackTarget',
        // 添加绩效字段
        'filledValue', 'filledText', 'kpiStatus', 'kpiScore', 'feedback', 'evidenceFiles',
    ];
    ...
    // 特数记录类型
    static RECORD_PRETEST = new ObjectId('000000000000000000000000'); // 预测试
    static RECORD_GENERATE = new ObjectId('000000000000000000000001'); // 生成题
    ...
    args: {
        ...
        const data: RecordDoc = {
            ...
            judgeAt: null,
            // 添加绩效字段
            filledValue: args.filledValue,
            filledText: args.filledText,
            kpiStatus: args.kpiStatus || 'pending',
